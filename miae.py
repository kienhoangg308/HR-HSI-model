...
class MIAE(DataInfo):
    def __init__(self, ndata, nratio, nsnr=0, psf=None, srf=None, edm_num=80, stage=3):
        ...
        # self.patch_size = 50 * self.ratio  # divide the image into patches to accelerate training
        # self.patch_stride = 30 * self.ratio  # make sure the patches are overlapped so that all pixels are included
        self.patch_size = 50 * 2  # divide the image into patches to accelerate training
        self.patch_stride = 30 * 2  # make sure the patches are overlapped so that all pixels are included
    ...
    def set_batch_size(self):
        ...
        if self.ratio == 16:
            batch_size = 5
        # batch_size = (self.height // 512) * (self.width // 512) * batch_size
        batch_size = 25
        return batch_size
...
if __name__ == '__main__':
    ...
    # saved_data = sio.loadmat('C:/Users/Admin/Downloads/official data/BR.mat')
    saved_data = sio.loadmat('/home/student11/HR-HSI-model/BR.mat')
    psf = torch.tensor(saved_data['B'])
    srf = torch.tensor(saved_data['R'])
    net = MIAE(ndata=ndata, nratio=nratio, nsnr=nsnr, psf=psf, srf=srf, stage=stage, edm_num=edm_num)
